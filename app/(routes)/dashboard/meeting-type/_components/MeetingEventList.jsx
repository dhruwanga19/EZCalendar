...
function MeetingEventList() {
    ...
    const getEventList = async () => {
        setEventList([]);
        const q = query(collection(db, "meetingEvents"),
    ...
    const onDeleteMeetingEvent=async(event)=>{
      await deleteDoc(doc(db, "meetingEvents", event?.id)).then(resp=>{
    ...
    return (
        ...
        lg:grid-cols-3 gap-7'>
            {eventList.length>0?eventList?.map((event,index)=>(
                ...
                style={{borderTopColor:event?.theme}}
                key={index}
                >
                    ...
                    <h2 className='flex gap-2 text-gray-500'><MapPin/> {event.location} Meet </h2>
